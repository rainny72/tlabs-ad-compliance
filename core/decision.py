...
from shared.regional_policies.base import get_regional_severity, get_strictest_severity, RegionalPolicy
...
def _evaluate_compliance(
    ...
    for v in violations:
        ...
        if region == Region.GLOBAL:
            # Global: apply strictest severity across all regions
            ...
            # Try to match each violation's evidence to a specific sub-rule
            matched_sub_rule = None
            for ev in v.violations:
                combined_text = f"{ev.description} {ev.evidence}"
